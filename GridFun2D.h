...
public:
    ...
    double hy, yMin, yMax;
    long xPanel, yPanel;
    GridFun2D(){initialize();};
    GridFun2D(long, double, double, long, double, double);
    GridFun2D(const GridFun2D&);
    virtual ~GridFun2D(){};
    void initialize();
    void initialize(const GridFun2D&);
    void initialize(long, double, double, long, double, double);
    void operator=(const GridFun2D&);
    void operator+=(const GridFun2D&);
    void operator-=(const GridFun2D&);
    ...
    double normInf();
    friend std::ostream& operator<<(std::ostream&, const GridFun2D&);
    void extractXslice(const long, std::vector<double>&);
    void insertXslice(const long, std::vector<double>&);
    void extractYslice(const long, std::vector<double>&);
    void insertYslice(const long, std::vector<double>&);
};
